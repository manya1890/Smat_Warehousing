...
class Database_common_operations:
    ...
    def create_role_table(cls):
        ...
        """
        query = f"""
            create table  IF NOT EXISTS Role (Role_id varchar2 PRIMARY KEY, Role_Name varchar2);
        """
        cls.run_query(query)
    ...
    def create_order_table(cls):
        query = f"create table IF NOT EXISTS Order_t (Order_id varchar2 PRIMARY KEY, Buyer_id, P_id varchar2, Quantity number, Quantity_unit varchar2, FOREIGN KEY(P_id) REFERENCES Product(P_id));"
        res = Database_common_operations.run_query(query)
        if res:
    ...
    def create_bill_table(cls):
        query = "create table IF NOT EXISTS Bill (Bill_id varchar2 PRIMARY KEY, Bill_amt number, Date_Time DATE, Order_id varchar2);"
        res = Database_common_operations.run_query(query)
        if res:
    ...
    def create_warehouse_space_table(cls):
        query = "create table IF NOT EXISTS Warehouse_Space (Location_id varchar2 PRIMARY KEY, Is_empty BOOLEAN, P_id varchar2, L_Dimensions varchar2, For_fragile BOOLEAN, FOREIGN KEY(P_id) REFERENCES Product(P_id));"
        res = Database_common_operations.run_query(query)
        if res:
            print("Bill Table Created!")
    ...
    def create_sales_return_table(cls):
        query = "create table IF NOT EXISTS Sales_Return (Return_id varchar2 PRIMARY KEY, Bill_id varchar2, Quantity_Return number, Order_id varchar2, FOREIGN KEY(Bill_id) REFERENCES Bill(Bill_id));"
        res = Database_common_operations.run_query(query)
        if res:
    ...
    def create_purchase_table(cls):
        query = "create table IF NOT EXISTS Purchase (Purchase_id varchar2, P_id varchar2, Quantity number, Amount number, S_id varchar2, Date_Time DATE, FOREIGN KEY(P_id) REFERENCES Product(P_id));"
        res = Database_common_operations.run_query(query)
        if res:
    ...
    def create_purchase_return(cls):
        query = "create table IF NOT EXISTS Purchase_Return(P_return_id varchar2 PRIMARY KEY, Purchase_id varchar2, FOREIGN KEY(Purchase_id) REFERENCES Purchase(Purchase_id));"
        res = Database_common_operations.run_query(query)
        if res:
...
def add_role(role_id, role_name: str):
    """
    Add type of role
    :return: None
    """
    ...
    id = role_id
    query = f"""
        insert into Role values('{id}','{tmp}');   
    """
    res = Database_common_operations.run_query(query)
    if res:
        print("Role Added")
...
def add_order(buyer_id: str, P_id: str, quantity, quantity_unit: str):
    o_id = "O"+str(Database_common_operations.generate_id())
    query = f"insert into Order_t values('{o_id}','{buyer_id}','{P_id}','{quantity}','{quantity_unit}');"
    res = Database_common_operations.run_query(query)
    if res:
        print("Order added!")
...
def remove_product(p_id: str):
    query = f"delete from Product where P_id == {p_id};"
    res = Database_common_operations.run_query(query)
    if query:
        print("Product Removed!")

def remove_category(c_id: str):
    query = f"delete from Category where Category_id == {c_id};"
    res = Database_common_operations.run_query(query)
    if res:
...
def sign_up(email: str, user_name: str, ph_number, opt_number, password: str, post: str, gender: str, birth, address: str):
    """
    Sign's up new user.

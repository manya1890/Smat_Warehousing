...
class Database_common_operations:
    ...
    def create_userDetails_table(cls):
        ...
        query = f"""
            create table user_details(id varchar2 PRIMARY KEY, email varchar2, user_details_name varchar2, phn_number number(10), post varchar2, FOREIGN KEY(post) REFERENCES Role(Role_Name) ON DELETE CASCADE);
        """
...
def add_user(email, user_details_name, phn_number, post):
    """
    ...
    query = f"""
        insert into user_details values('{id}','{email}','{user_details_name}',{phn_number}, '{post}');
    """
    Database_common_operations.run_query(query)
...
def sign_up(email: str, user_name: str, phn_number, password: str, post: str):
    """
    Sign's up new user.
    ...
    id = add_user(email, user_name, phn_number, post)
...
Database_common_operations.run_query("UPDATE ids set id='2' where id='3';")
